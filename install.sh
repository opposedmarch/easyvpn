...
# Root
[[ $(id -u) != 0 ]] && echo -e "\n You should have ${red}root ${none}acess to run this script. Please use sudo or sudo su ${yellow}~(^_^) ${none}\n" && exit 1
...
*)
	echo -e " 
	${red}Error:${none} Your system is not supported! ${yellow}(-_-) ${none}

	Only Ubuntu 16+ / Debian 8+ / CentOS 7+ are supported.
	" && exit 1
	;;
esac
...
_sys_timezone() {
	...
	echo "Your host has been set to Asia/Shanghai timezone and automatically synced with systemd-timesyncd." #TODO: Why it is set to Asia timezone?
	echo
	...
	#   echo -e "Warning: The ${yellow}Openvz${none} system time cannot be synchronized by the program inside a virtual machine."
	...
	#   echo -e "If the host time differs from the actual host by ${yellow} by more than 90 seconds ${none}, v2ray will not be able to communicate normally. Please send a ticket to contact the vps host company for adjustment."
		echo -e "Your VPS provider is using Openvz. Therefore, it was not pssible to configure the system time."
		echo -e "If the host time differs more than 90 seconds with the acutal time, v2ray will not be able to communicate normally"
	fi
}
...
v2ray_config() {
	...
	while :; do
		...
		# echo -e "Please select "$yellow"V2Ray"$none" communication protocol [${magenta}1-${#transport[*]}$none]"
		-------------------------
		echo -e "Please select "$yellow"V2Ray"$none" communication protocol [${magenta}1-${#transport[*]}$none]"
		echo
		...
		echo "Note 2:  [utp | srtp | wechat-video | dtls | wireguard] respectively disguised as [BT download | video call | WeChat video call | DTLS 1.2 packet | WireGuard packet]"
		echo
		read -p "$(echo -e "(Default protocol: ${cyan}WebSocket$none)"):" v2ray_transport
		[ -z "$v2ray_transport" ] && v2ray_transport=3
		case $v2ray_transport in
		[1-9] | [1-2][0-9] | 3[0-3])
			echo
			echo
			echo -e "$yellow V2Ray communication protocol = $cyan${transport[$v2ray_transport - 1]}$none"
			echo "----------------------------------------------------------------"
			echo
			break
			;;
...
tls_config() {
	...
	while :; do
		...
		80)
			...
			echo " ...I already said you can't choose port 80......"
			error
			;;
		443)
			echo
			# echo " ..都说了不能选择 443 端口了咯....."
			echo " ..I already said you can't choose port 443....."
	...
	while :; do
		...
		echo -e "Please enter a ${magenta}correct domain name${none}, it must must must be correct, CANNOT! BE! WRONG!"
	...
	while :; do
		...
		if [[ -z "$record" ]]; then
			...
			if [[ "$record" == [Yy] ]]; then
				...
				echo -e "$yellow DNS parsing = ${cyan}I'm sure it's already parsed$none"
...
auto_tls_config() {
	...
	echo -e "
		...
		$yellowAnd.. you can do the configuration of TLS yourself $none

		Then you don't need to turn on automatic configuration for TLS
		"
	echo "----------------------------------------------------------------"
	echo
...
path_config() {
	...
	while :; do
		...
		echo -e "Please enter the path you want ${magenta} to use for diversion $none , such as /233blog. If so, then just enter 233blog"
		...
		*[/$]*)
			...
			echo -e " Because this script is too stupid.. so the path of the diversion cannot contain the symbols $red / $none or $red $ $none.... "
			echo
			error
			;;
		*)
...
proxy_site_config() {
	...
	while :; do
		...
		echo -e "Please enter ${magenta} a correct $none ${cyan} URL$none to be used as a masquerade for ${cyan} sites$none , for example https://liyafly.com"
		# echo -e "举例...你当前的域名是 $green$domain$none , 伪装的网址的是 https://liyafly.com"
		echo -e "For example, when your current domain name is $green$domain$none , the fake URL is https://liyafly.com"
		...
		echo -e "In fact, it is an reverse proxy... It's good that you understand..."
...
blocked_hosts() {
	...
	while :; do
		...
		Y | y)
			...
			echo -e "$yellow ad blocking = $cyan on $none"
			echo "----------------------------------------------------------------"
			echo
			break
			;;
...
shadowsocks_port_config() {
	...
	while :; do
		...
		echo -e "Please enter "$yellow"Shadowsocks"$none" port ["$magenta"1-65535"$none"], cannot be the same port as "$yellow"V2Ray"$none""
		...
		$v2ray_port)
			echo
			# echo " 不能和 V2Ray 端口一毛一样...."
			echo " Cannot be exactly the same port as V2Ray ...."
			error
			;;
		[1-9] | [1-9][0-9] | [1-9][0-9][0-9] | [1-9][0-9][0-9][0-9] | [1-5][0-9][0-9][0-9][0-9] | 6[0-4][0-9][0-9][0-9] | 65[0-4][0-9][0-9] | 655[0-3][0-5])
...
shadowsocks_password_config() {
	...
	while :; do
		...
		*[/$]*)
			...
			echo -e " Since this script is too stupid.. so the password cannot contain symbols $red / $none或$red $ $none "
			echo
			error
			;;
		*)
...
domain_check() {
	...
	if [[ $test_domain != $ip ]]; then
		...
		echo
		echo -e " your domain name: $yellow$domain$none not resolved: $cyan$ip$none"
		echo
		echo -e " Your domain name currently resolves to: $cyan$test_domain$none"
		echo
		echo "Note...if your domain is resolved with Cloudflare..click on that icon in Status..make it greyed out"
		echo
		exit 1
	fi
}
...
install_v2ray() {
	...
	if [[ $local_install ]]; then
		...
		if [[ ! -d $(pwd)/config ]]; then
			...
			echo
			echo -e " Please make sure you have the complete V2Ray one-click installation script & management script uploaded to 233v2.com to the current ${green}$(pwd) $none directory."
			echo
			exit 1
		fi
...
get_ip() {
	...
	[[ -z $ip ]] && ip=$(curl -s icanhazip.com)
	[[ -z $ip ]] && ip=$(curl -s myip.ipip.net | grep -oE "([0-9]{1,3}\.){3}[0-9]{1,3}")
	[[ -z $ip ]] && echo -e "\n$red Throw away this rubbish!$none\n" && exit
}
...
install() {
	if [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/233boy/v2ray ]]; then
		echo
		echo " Hi there...you already installed V2Ray...no need to reinstall"
		echo
...
uninstall() {
	...
	if [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/233boy/v2ray ]]; then
		...
		echo -e "
		$red You don't seem to have V2Ray installed....Cannot uninstall...$none
...
*)
	...
	echo -e " The parameter you entered <$red $args $none>...what the h*** is this...the script doesn't recognize it wow"
	echo
	# echo -e " 这个辣鸡脚本仅支持输入$green local / online $none参数"
	echo -e " The stupid script only support entering parameter $green local / online $none"
	echo
...
while :; do
	...
	echo "........... V2Ray One-Click Install Script & Admin Script by 233v2.com .........."
	echo
	...
	read -p "$(echo -e "Please enter [${magenta}1-2$none]:")" choose
	case $choose in
	1)
		install
		break
